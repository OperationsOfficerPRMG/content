...
- `method`
  ...
  - : The [HTTP](/en-US/docs/Web/HTTP) method to submit the form with.
    ...
    - `get` (default): The {{HTTPMethod("GET")}} method; form data appended to the `action` URL with a `?` separator. Use this method when the form [has no side effects](/en-US/docs/Glossary/Idempotent) and will not include any of the user's personal data.
